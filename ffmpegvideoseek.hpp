...
static ImplMutex _mymutex;
static bool _initialized = false;

class MyAutoLock
{
public:
	MyAutoLock(ImplMutex& m) : mutex(&m) { mutex->lock(); }
	~MyAutoLock() { mutex->unlock(); }
protected:
	ImplMutex* mutex;
private:
	MyAutoLock(const MyAutoLock&); // disabled
	MyAutoLock& operator = (const MyAutoLock&); // disabled
};
